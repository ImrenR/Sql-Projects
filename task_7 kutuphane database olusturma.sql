...
select * from tur;
-- Task01 -> sayfa sayisi  200'den büyük olan kitaplari bilgileri listeleyen query create ediniz.
select * from kitap
where sayfasayisi > 200;

-- Task02 -> yazar no 45 olan kitabin sayisi listeleyen query create ediniz.
select sayi
from kitap
where yazarno = 45;

-- Task03 -> yazar no su 4 ile 9 arasinda olan yazar isimlerini listeleyen query create ediniz.

select ad,soyad
from yazar
where yazarno between 4 and 9;  -- between ile ayni sorunu n cozumu

select ad,soyad from yazar where yazarno>=4 and yazarno<=9; -- and yerine && de kullanilabilir



-- Task04 -> charles dickens ile cengiz aytmatov arasindaki yazarlari listeleyen query create ediniz.

select yazarno,ad,soyad
from yazar
where ad
between 'charles' and 'turgut'; -- bu sartimizda c ve t(t dahil degil) harfleri arasindaki yazar isimleriniz yazdirir

-- Task05 -> sayfa sayisi 200 den buyuk kitap ismi 'beyaz dis' olan kitabin ismi puanini listeleyen query create ediniz.
select ad,puan
from kitap
where sayfasayisi > 200 and ad = 'beyaz dis';

-- Task06 -> kitap no su 98 54 21 76 olan kitapno ve isimlerini listeleyen query create ediniz.
select kitapno,ad
from kitap
where kitapno in (98,54,21,76);

-- Task07 -> İsmi A harfi ile başlayan yazarlari listeleyen query create ediniz.
...
-- Task08 -> İsminin 2. harfi E olan yazarin listeleyen query create ediniz.
select ad from yazar where ad like '_e%';

-- Task09 -> İsminin 2. harfi E olup diğer harflerinde Y olan yazarlari listeleyen query create ediniz.
 select ad from yazar where ad like '_e%y%';    -- ilk harf bos 2. harf e iki % arasi kelimenin tamamindaki harfi verir

-- Task10 -> isim ilk harfi r ile baslayip ikinci harfi belirsiz ve icinde k harfi bulunan isimleri listeleyen query create ediniz.
select ad from yazar where ad like 'r_%k%' ;     -- _ bu harfi atla demek

-- Task11 -> isminde a harfi olmayan isimleri listeleyen query create ediniz.
select ad from yazar where ad not like '%a%';

-- Task12 -> kitab adi 6 haneli olan kitablari listeleyen query create ediniz.
